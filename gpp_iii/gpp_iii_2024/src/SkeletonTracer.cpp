...
void SkeletonTracer::collateRawDrawingIntoLabeledSkeletons(const std::vector<PersonContour>& personContours){
    ...
    if ((nBones > 0) && (nPersonContours > 0)){
        std::sort(theRawDrawing.begin(), theRawDrawing.end(), comparePolylinePluses);
        ...
        for (int i=0; i<nBones; i++){
            ...
            if (nBonePoints > 0){
                ...
                if (nBonePoints > MAX_POINTS_PER_BONE){
                    aBonePolyline = aBonePolyline.getResampledByCount(MAX_POINTS_PER_BONE);
                }
...
//------------------------------------------------------------
bool SkeletonTracer::comparePolylinePluses(const PolylinePlus& a, const PolylinePlus& b) {
    return a.polyline.size() > b.polyline.size(); // Compare by size, longest to shortest
}

...
class PoissonFill{public:
  ...
  void init(int _w, int _h, int _depth){
    ...
    feedbackTwiddle = 0.03;
    
    for (int i = 0; i < depth; i++){
      _w /= 2;
      _h /= 2;
        downs[i].allocate(_w,_h, GL_RGBA16F); //GL_RGBA);
    }
    for (int i = 0; i < depth; i++){
      _w *= 2;
      _h *= 2;
      ups[i].allocate(_w,_h, GL_RGBA16F); //GL_RGBA);
    }
    shader = shader2way();
  }
  ...
  void pass(ofFbo& p, ofTexture* tex1, ofTexture* tex2){
    ...
    // shader.setUniform1i("h",tex1->getHeight());
      ...
      float feedbackChanger = feedbackTwiddle; //0.03 ... -0.05 ;  0.04*sin(ofGetElapsedTimef()/5.0) - 0.01;

...
//-------------------------------------------------------------------------------
void SkeletonBoneDrawer::drawScaledBone (Bone &bone, float thickness){
    
    mesh.clear();
    mesh.setMode(OF_PRIMITIVE_TRIANGLE_STRIP);

    int nQuads = 0;
    int nPoints = bone.nPoints;
    if (nPoints >= 3){
        ...
        // draw a startcap
        capx = bone.points[0].x;
        capy = bone.points[0].y;
        ofDrawCircle(capx, capy, radius0);
        ...
        for (i=1; i<nPathPoints; i++){
            ...
            p0x = (float) (bone.points[i-1].x);
            p0y = (float) (bone.points[i-1].y);
            p1x = (float) (bone.points[i  ].x);
            p1y = (float) (bone.points[i  ].y);
            p2x = (float) (bone.points[i+1].x);
            p2y = (float) (bone.points[i+1].y);
        ...
        // handle the last point
        p2x = bone.points[nPathPoints].x;
        p2y = bone.points[nPathPoints].y;
...
//-------------------------------------------------------------------------------
void SkeletonBoneDrawer::drawScaledDotted (ofPolyline &bone, float thickness){
    
    int nPoints = (int) bone.size();
    if (nPoints >= 3){
        for (int i=0; i<nPoints; i+=3){
            float p0x = (float) (bone[i].x);
            float p0y = (float) (bone[i].y);
            ofDrawCircle(p0x, p0y, thickness);
        }
    }
}
...
//-------------------------------------------------------------------------------
void SkeletonBoneDrawer::drawScaledDottedBone (Bone &bone, float thickness){
    
    int nPoints = bone.nPoints;
    if (nPoints >= 3){
        
        for (int i=0; i<nPoints; i+=3){
            float p0x = (float) (bone.points[i].x);
            float p0y = (float) (bone.points[i].y);
            ofDrawCircle(p0x, p0y, thickness);
        }
    }
}

...
// Meshy is an interactive drawing environment in which the user's
// strokes scaffold a gauzy mesh of animated elements. The mesh
// continually bridges the user's two most recent movements.
...
const maxNPaths=2;
const mult=5;
const nBezSamp=75*mult;
const quadDim=0.0035*mult;
const nResamp=75;
...
let bBegun=false;
let bJustLaunched=true;
let bShowInfo=true;
let bSpecialCase=false;
let bDrawDebug=false;
let bAuto=false;
let bEnableAuto=true;
let bMPressed=false;
...
canvas.width=myW*pxRat;
canvas.height=myH*pxRat;
canvas.style.width=myW+'px';
canvas.style.height=myH+'px';
ctx=canvas.getContext('2d');
ctx.scale(pxRat,pxRat);
}

function initMeshy(){
currSamp=nClicks=currPathI=0;
fSinceMUp=0;
helpStartT=myMils();
pathArr=new Array(maxNPaths);
pathBezArr=new Array(maxNPaths);
...
omt2[p]=omt[p]*omt[p];
omt3[p]=omt[p]*omt2[p];
bto2[p]=3*bt[p]*omt2[p];
bt2o[p]=3*bt2[p]*omt[p];
}
}

function myDraw(){
ctx.fillStyle='black';
ctx.fillRect(0,0,myW,myH);
simulate();
render();
drawDebug();
drawInfo();
...
function autoPlay(){
  if(bEnableAuto){
    let prevbAuto=bAuto; 
    let elapsed=myMils()-lastPokeT;
    bAuto=(elapsed>waitToAutoDur);
    let splen=autoLen-auDel;
    let pt;
    if(bAuto&&!prevbAuto){
      currAutoF=0;autoPress();
    }else if(!bAuto&&prevbAuto){
      pt=getAutoPt(currAutoF/splen);
      myMouseReleased(pt.x,pt.y);
    }else if(bAuto){
      if (currAutoF<autoLen){
        currAutoF++;
        if(currAutoF<splen){
          pt=getAutoPt(currAutoF/splen);
          myMouseDragged(pt.x,pt.y);}
      }else{
        pt=getAutoPt(currAutoF/splen);
        myMouseReleased(pt.x,pt.y);
        autoPress();
...
let px=pathArr[j][i].x;
let py=pathArr[j][i].y;
if(i==0){ctx.moveTo(px,py);}
ctx.lineTo(px,py);
}
ctx.stroke();
}
let elapsed=myMils()-startDebugT; 
if(bAuto||(elapsed>120000)){
bDrawDebug=false;
}}}
...
let A=0.025;
let B=1-2*A;
let path=pathArr[currPathI];
let npm1=path.length-1;
if(npm1>2){
for(let i=1;i<npm1;i++){
let v0=path[i-1],v1=path[i],v2=path[i+1];
let px=A*v0.x+B*v1.x+A*v2.x;
let py=A*v0.y+B*v1.y+A*v2.y;
let pz=A*v0.z+B*v1.z+A*v2.z;
path[i].set(px,py,pz);
}
...
function render(){
	if(nClicks>0){
		...
		if(nClicks!=1||currPathI!=1||bSpecialCase){
			...
			for(let i=0;i<=num;i++){
				fromI=(i-1+offset+nPaths)%nPaths;
				toI=(i+offset+nPaths)%nPaths;
				if(toI%2==1){
					let fromArr=drawArr[fromI];
					let toArr=drawArr[toI];
					if(nPaths==2||
						currPathI!=(fromI-offset)%nPaths){
						drawMesh(fromArr,toArr);
					}
				} 
			}
		}
	}else{
		let n=Math.min(nClicks,nPaths);
		for(let p=0;p<=n;p++){
			drawStem(pathBezArr[p]);
		}
	}
}
...
function drawInfo(){
if(bShowInfo){
ctx.font=txSize+"px 'Times New Roman'";
...
function createStarter(){	
	...
	for(let g=0;g<2;g++){
		...
		for(let i=0;i<aGest.length;i++){
			let p=aGest[i];
			pathArr[currPathI].push(new Vec3f(p.x,p.y,0));
		}
		calcBezPath(currPathI);
		resampVec(currPathI);
		nClicks++;
...
let p2=new Vec3f();
let p3=new Vec3f();
let last_pt=new Vec3f();
let next_pt=new Vec3f();
if(nSourcePts<5){
...
let dx0=p0.x-last_pt.x;
let dy0=p0.y-last_pt.y;
let d0=Math.sqrt(dx0*dx0+dy0*dy0);
let dx1=p3.x-p0.x;
let dy1=p3.y-p0.y;
let d1=Math.sqrt(dx1*dx1+dy1*dy1);
let dx2=next_pt.x-p3.x;
let dy2=next_pt.y-p3.y;
let d2=Math.sqrt(dx2*dx2+dy2*dy2);
let dAtp0=(d0+d1)/2;
let dAtp3=(d1+d2)/2;
p0.set(p0.x,p0.y,dAtp0);
p3.set(p3.x,p3.y,dAtp3);
let tan_inx=(p3.x-last_pt.x)/bias;
let tan_iny=(p3.y-last_pt.y)/bias;
let tan_inz=(p3.z-d0)/bias;
let tan_outx=(next_pt.x-p0.x)/bias;
let tan_outy=(next_pt.y-p0.y)/bias;
let tan_outz=(d2-p0.z)/bias;
let p1x=p0.x+tan_inx;
let p1y=p0.y+tan_iny;
let p1z=p0.z+tan_inz;
let p2x=p3.x-tan_outx;
let p2y=p3.y-tan_outy;
let p2z=p3.z-tan_outz;
p1.set(p1x,p1y,p1z);
p2.set(p2x,p2y,p2z);
	...
	if(nPathPts>0){
		...
		if(nPathPts<=1){
			for(p=0;p<nResampPts;p++){
				let lo=path[0];
				let px=lo.x+p*0.0001;
				let py=lo.y+p*0.0001;
				let pz=lo.z+p*0.0001;
			...
			for(let i=0;i<nPathPts-1;i++){
				...
				if(ASL>=needsL){
					let remainder=ASL;
					let px=lo.x+needsL*dx;
					let py=lo.y+needsL*dy;
					let pz=lo.z+Dz/2;
					if(p<nResampPts){
						resampledPath[p].set(px,py,pz);
						remainder-=needsL;p++;
...
let p4a=toArr[i];
let p4b=toArr[i+1];
let p1x=(p1a.x+p1b.x)/2;
let p1y=(p1a.y+p1b.y)/2;
let p1z=(p1a.z+p1b.z)/2;
let p4x=(p4a.x+p4b.x)/2;
let p4y=(p4a.y+p4b.y)/2;
let p4z=(p4a.z+p4b.z)/2;
...
let sa=(s+roundSamp)%nBezSamp;
let sb=(s+roundSamp+1)%nBezSamp;
let omt3sa=omt3[sa];
let bto2sa=bto2[sa];
let bt2osa=bt2o[sa];
let bt3sa=bt3[sa];
...
let pt=new Vec3f(ptx,pty,ptz);
return pt;
}

function generateStarterGestures(){
	gestA=[];
	gestB=[];
	let mode=getOpt([ 
		[MWHALE,25],
	...
	if(mode==MWHALE){
		...
		let cPtsA=generateBezControlPointSet(nBezSetsA,baseA);
		let cPtsB=generateBezControlPointSet(nBezSetsB,baseB);
		gestA=generateBezGesture(cPtsA,nBezSetsA);
		gestB=generateBezGesture(cPtsB,nBezSetsB);
	}else if(mode==MSCRIB){
		gestA=generateScribbleGest();
		gestB=generateScribbleGest();
	}else if(mode==MSTEER){
		gestA=generateSteerGest(-1);
		gestB=generateSteerGest(1);
	}else if(mode==MMIXY1){
		gestA=generateSteerGest();
		gestB=generateScribbleGest();
	}else if(mode==MMIXY2){
		let nBezSetsB=2;
		let baseB=myRAB(0.4,0.6);
		let cPtsB=generateBezControlPointSet(nBezSetsB,baseB);
		gestB=generateBezGesture(cPtsB,nBezSetsB);
		gestA=generateScribbleGest();
	}else if(mode==MWHAL2){
		let nBezSetsA=getOpt([[1,5],[2,35],[3,40],[4,20]]);
		let baseA=myRAB(0.5,0.6);
		let cPtsA=generateBezControlPointSet(nBezSetsA,baseA);
		gestA=generateBezGesture(cPtsA,nBezSetsA);
		gestB=generateNearDupGesgture(gestA);
	}else if(mode==MSRIB2){
		gestA=generateScribbleGest();
		gestB=generateNearDupGesgture(gestA);
	}else if(mode==MSTER2){
...
function generateSteerGest(lr){
	...
	let t=myRAB(0,100);
	let ori=myRAB(0,tau);
	let dt=tau*myRAB(0.001,0.013);
	let dori=myRAB(0.08,0.2);
	let speed=myW/nSteerPts;
	for(let i=0;i<nSteerPts;i++){
		ori+=dori*myMap(myNoise(t),0,1,0-tau,tau)/2;
		let dx=speed*Math.cos(ori);
		let dy=speed*Math.sin(ori);
...
function generateBezGesture(cPts,nBezSets){
	let gestPts=[];
	let nPoints=60/nBezSets;
	for(let j=0;j<nBezSets;j++){
